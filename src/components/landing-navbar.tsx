...
export default function LandingNavbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 px-4 sm:px-6 pt-3">
      <div className={`max-w-6xl mx-auto bg-white/40 backdrop-blur-xl rounded-full border border-gray-200/50 transition-all duration-300 ${
        scrolled ? "bg-white/70 shadow-lg shadow-black/5" : ""
      }`}>
        <div className="flex items-center justify-between px-4 sm:px-6 h-12">
          ...
          <div className="hidden md:flex items-center gap-1">
            {links.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="text-[13px] font-medium text-gray-800 hover:text-gray-900 px-4 py-1.5 rounded-full hover:bg-black/5 transition-all duration-200 whitespace-nowrap"
              >
                {link.label}
              </a>
            ))}
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <Link
              href="/login"
              className="hidden sm:inline-flex text-[13px] font-medium text-gray-800 hover:text-gray-900 px-3 py-1.5 rounded-full hover:bg-black/5 transition-all duration-200"
            >
              Connexion
            </Link>
            <Link
            ...
            <button
              ...
              className="md:hidden w-8 h-8 flex items-center justify-center rounded-full hover:bg-black/5 transition-colors ml-1"
              aria-label="Menu"
            >
