...
const PROMPTS: PromptSection[] = [
  {
    ...
    description: "Modifie une ad existante selon l'instruction de l'utilisateur.",
    systemPrompt: `Images envoyées : l'ad existante à modifier`,
    userPromptTemplate: `Edit this ad: "{modificationPrompt}". Keep EVERYTHING else identical — same layout, colors, text, composition. Only apply the requested change. ALL text MUST be in French.`,
  },
  {
    id: "reference",
    title: "Mode Copy-Ads (Référence)",
    model: "Gemini 3.1 Flash Image Preview",
    icon: <Image className="w-4 h-4" />,
    description: "L'utilisateur upload une ad → on la recrée pour sa marque.",
    systemPrompt: `Images envoyées : ad de référence uploadée, photo produit, logo`,
    userPromptTemplate: `Recreate this ad for "{brandName}" selling "{productName}". Keep the EXACT same layout, composition, number of text elements, and visual structure as the reference. Replace: the brand → "{brandName}", the product → use the provided product photo. Copy the product photo EXACTLY as-is — same shape, colors, packaging. Never redesign it. Use the provided logo for "{brandName}" exactly as-is. Do NOT add any CTA button or call-to-action. Keep text minimal — only replace existing text, don't add more. Never invent fake prices or claims. [Story: leave top 15% and bottom 20% empty.] ALL text MUST be in French.`,
  },
  {
    id: "template",
    title: "Mode Bibliothèque / Auto",
    model: "Gemini 3.1 Flash Image Preview",
    icon: <Bot className="w-4 h-4" />,
    description: "Template de la BDD → on le recrée pour la marque du client.",
    systemPrompt: `Images envoyées : template de la bibliothèque, photo produit, logo`,
    userPromptTemplate: `Recreate this ad template for "{brandName}" selling "{productName}". CRITICAL: Keep the EXACT same layout — same number of text blocks, same text positions, same composition, same visual balance. Simply SWAP: the brand name → "{brandName}", the product → use the provided product photo, the headline → something about "{productName}" (2-5 words max). Copy the product photo EXACTLY as-is — same shape, colors, packaging. Never redesign it. Use the provided logo for "{brandName}" exactly as-is. Do NOT add any CTA button or call-to-action. Do NOT add extra text, prices, or elements that aren't in the template. Do NOT add decorative elements. Keep it clean and faithful to the template's structure. [Story: leave top 15% and bottom 20% empty.] ALL text MUST be in French.`,
  },
  {
    id: "fallback",
    title: "Mode Fallback (sans template)",
    model: "Gemini 3.1 Flash Image Preview",
    icon: <Cpu className="w-4 h-4" />,
    description: "Aucun template sélectionné → création from scratch.",
    systemPrompt: `Images envoyées : photo produit, logo`,
    userPromptTemplate: `Create a clean, professional Instagram ad for "{brandName}" selling "{productName}". Copy the product photo EXACTLY as-is. Use the provided logo. Short headline (2-5 words). Do NOT add any CTA button. Minimal, modern design. [Story: leave top 15% and bottom 20% empty.] ALL text MUST be in French.`,
  },
  {
    id: "config",
    ...
    systemPrompt: `Modèle : gemini-3.1-flash-image-preview\nResponse modalities : ["IMAGE", "TEXT"]\nTimeout : 55 secondes\nRetries : 1 (côté serveur) + 2 (côté client)\nAspect ratio : 1:1 (carré) ou 9:16 (story)`,
    userPromptTemplate: `Les images de référence sont envoyées AVANT le texte du prompt (Gemini traite mieux les images en premier).\n\nOrdre des images :\n1. Template ou ad de référence (direction créative)\n2. Photo produit\n3. Logo de la marque\n\nChaque image a un label explicatif.`,
  },
];
